...
def video_process(frame_queue1, frame_queue2, darknet_image_queue1, darknet_image_queue2):
	...
	while cap1.isOpened() and cap2.isOpened(): #while cap1.isOpened() or cap2.isOpened():
		...
		#print(f"shape of the frame:: {frame_rgb1.shape}")
		frame_resized1 = cv2.resize(frame_rgb1, (darknet_width, darknet_height),
					interpolation=cv2.INTER_LINEAR)
		...
		frame_queue1.put(frame_resized1)
		frame_queue2.put(frame_resized2)  
		    
		#darknet_image_queue1.put(frame_resized1)
		#darknet_image_queue2.put(frame_resized2)
		...
		darknet.copy_image_from_bytes(img_for_detect1, frame_resized1.tobytes())
		darknet.copy_image_from_bytes(img_for_detect2, frame_resized1.tobytes())
		
		darknet_image_queue1.put(img_for_detect1)
		darknet_image_queue2.put(img_for_detect2)
		#global stop_threads
...
def inference(darknet_image_queue1,darknet_image_queue2, detections_queue1, detections_queue2):
	while cap1.isOpened() or cap2.isOpened():
		...
		detections1 = darknet.detect_image(network, class_names, darknet_image1, thresh=args.thresh)
		detections2 = darknet.detect_image(network, class_names, darknet_image2, thresh=args.thresh)
		...
		if not detections_queue2.empty():
				try:
					...
					pass
		print(f"the detections are :: {detections1}")
		detections_queue1.put(detections1)
		detections_queue2.put(detections2)
...
def drawing(frame_queue1, frame_queue2, object_queue1, object_queue2):
	...
	width, height = 608, 608#1920, 1080
	...
	while cap1.isOpened() and cap2.isOpened():
		...
		if frame1 is not None and frame2 is not None:
			...
			for obj, label, score, name in objects1:
				...
				#print(f"the detections are:: {len(obj)}")
...
if __name__ == '__main__':
	...
	cap2 = EasyPySpin.VideoCapture(1)
